...
const C = {
  ...
  medBlue: "#007AFF", medBlueBg: "#E8F4FF",
  red: "#FF3B30", redBg: "#FFF0EF", green: "#34C759", greenBg: "#EDFAF0",
  purple: "#AF52DE", purpleBg: "#F8F0FF", orange: "#FF9F0A", orangeBg: "#FFF8ED",
  shadow: "0 2px 16px rgba(0,0,0,0.06)", shadowHover: "0 8px 32px rgba(0,0,0,0.1)",
};
...
export default function TrainerAssignments() {
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "800px", margin: "0 auto", minHeight: "calc(100vh - 100px)", display: "flex", flexDirection: "column" }}>
      ...
      {/* Header — full width, button pushed to right */}
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 28, flexShrink: 0 }}>
        ...
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, flex: 1, letterSpacing: "-0.5px" }}>Assignments</h1>
      ...
      {/* Empty State — centered with balanced spacing */}
      {assignments.length === 0 && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "40px 24px" }}>
          ...
          <h3 style={{ margin: "0 0 10px", fontSize: 20, fontWeight: 700, letterSpacing: "-0.3px" }}>No Assignments Yet</h3>
          <p style={{ margin: "0 0 32px", color: C.textTertiary, fontSize: 15, lineHeight: 1.5, maxWidth: 320 }}>Create your first assignment to get started.</p>
          <button onClick={() => navigate(`/trainer/assignment-builder/${courseId}`)} style={{ padding: "14px 36px", background: `linear-gradient(135deg, ${C.medBlue}, #0055D4)`, color: "#fff", border: "none", borderRadius: 14, fontWeight: 700, cursor: "pointer", fontSize: 16, boxShadow: "0 4px 16px rgba(0,122,255,0.35)", transition: "all 0.2s", letterSpacing: "0.3px" }}>Create Assignment</button>
        </div>
      )}

      {/* Assignment List — fills remaining space, scrollable */}
      {assignments.length > 0 && (
