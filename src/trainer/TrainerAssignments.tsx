      ...
      <div style={{ padding: "40px 48px 40px", width: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, width: "100%" }}>
          {assignments.map((a) => {
            ...
            return (
              <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 16, width: "100%" }}>
                <div style={{ ...TS.h2, width: 52, height: 52, borderRadius: 14, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: allGraded ? C.greenBg : C.purpleBg, color: allGraded ? C.green : C.purple, border: `1px solid ${C.separator}` }}>
                  {allGraded ? (
                ...
                <div onClick={() => navigate(`/trainer/assignments/${courseId}/${a.id}`)} style={{ flex: 1, background: C.bg, borderRadius: 12, padding: "18px 24px", cursor: "pointer", border: `1px solid ${allGraded ? C.green + "33" : C.separator}`, minWidth: 0, transition: "border-color 0.2s" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = allGraded ? C.green + "66" : C.medBlue + "33"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = allGraded ? C.green + "33" : C.separator; }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                    <span style={{ ...TS.caption, color: allGraded ? C.green : C.medBlue }}>{allGraded ? "All Graded" : "Submissions"}</span>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.textTertiary} strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg>
                  </div>
                  <h3 style={{ ...TS.h3, margin: 0 }}>{a.title}</h3>
                  <p style={{ ...TS.bodySm, margin: "4px 0 12px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{a.description || "No description"}</p>
                  {a.submission_count > 0 ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <span style={{ ...TS.label, fontSize: 12, fontWeight: 600, whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" }}>{a.graded_count}/{a.submission_count} graded</span>
                    </div>
                  ) : (
                    <span style={{ ...TS.label, fontSize: 12, fontWeight: 500 }}>No submissions yet</span>
                  )}
                </div>

                <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                  {a.submission_count > 0 && (
                    <button onClick={() => navigate(`/trainer/submissions/${a.id}`)} style={{ ...TS.input, padding: "8px 16px", background: C.greenBg, color: C.green, border: `1px solid ${C.green}22`, borderRadius: 10, fontWeight: 600, cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>Grade</button>
                  )}
