// src/trainer/TrainerDashboard.tsx
...
function mapCourseFromSupabase(c: Record<string, unknown>): Course {
  return {
    ...
    tuitionType: ((c.tuition_type as string) || "free") as "free" | "paid",
    amount: (c.amount as number) || 0,
    startDate: (c.start_date as string) || "",
    period: (c.period as string) || "",
    mediaUrl: (c.media_url as string) || (c.logo as string) || "",
    mediaType: (c.media_type as string) || "image",
    mediaName: (c.media_name as string) || "course-logo",
  };
}

function SkeletonCourseCard() {
  return (
    <div style={{ background: C.card, borderRadius: 24, boxShadow: C.shadow, overflow: "hidden", border: `1px solid ${C.separatorLight}` }}>
      <div style={{ width: "100%", height: 180, background: C.separatorLight }} />
      <div style={{ padding: 20, borderBottom: `1px solid ${C.separatorLight}`, flex: 1 }}>
        <div style={{ width: "60%", height: 16, borderRadius: 6, background: C.separator, marginBottom: 12 }} />
        <div style={{ width: "90%", height: 12, borderRadius: 4, background: C.separator, marginBottom: 6 }} />
        <div style={{ width: "40%", height: 12, borderRadius: 4, background: C.separator }} />
      </div>
      <div style={{ padding: 16, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
        {Array.from({ length: 4 }).map((_, i) => <div key={i} style={{ height: 56, borderRadius: 12, background: C.separatorLight }} />)}
      </div>
    </div>
  );
}
...
export default function TrainerDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const savedUser: Record<string, unknown> = JSON.parse(savedUserString);
      if (!savedUser.id || !savedUser.tenantId) { navigate("/"); return; }
      
      setCurrentUser(savedUser as unknown as User);
      
      try {
        const tenantId = savedUser.tenantId as string;
        const { data: s } = await supabase.from("business_settings").select("*").eq("tenant_id", tenantId).maybeSingle();
        if (s) {
          setSettings({
            id: s.id, businessName: s.business_name || "", address: s.address || "", phone: s.phone || "", email: s.email || "", website: s.website || "",
            logo: s.logo || "", header: s.header || "", adminProfile: s.admin_profile || "", loginBackground: s.login_background || "", themeColor: s.theme_color || "",
            appBarItems: s.app_bar_items || [], tenantId: s.tenant_id, createdAt: s.created_at,
          });
        }
        
        const { data: u } = await supabase.from("profile_settings").select("*").eq("user_id", savedUser.id).maybeSingle();
        if (u) {
          setCurrentUser((prev) => prev ? { ...prev, profilePic: u.avatar_url || prev.profilePic, username: u.username || prev.username } : prev);
        }
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      try {
        const { data: enrollments } = await supabase.from("enrollments").select("course_id").eq("user_id", currentUser.id).eq("role", "trainer").eq("tenant_id", tenantId);
        if (enrollments && enrollments.length > 0) {
          const courseIds = enrollments.map((e) => e.course_id);
          const { data: coursesRes } = await supabase.from("courses").select("*").in("id", courseIds).order("created_at", { ascending: false });
          if (coursesRes) coursesData = coursesRes.map(mapCourseFromSupabase);
        }
      } catch (e) { console.warn("Enrollments fetch failed", e); }
      
      const assigned = currentUser.assignedCourses;
      if (coursesData.length === 0 && assigned && assigned.length > 0) {
    ...
    } catch (err) { console.error("Failed to fetch courses", err); } finally { setLoading(false); }
  ...
  const iconBtnStyle: React.CSSProperties = {
    display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "12px 4px",
    borderRadius: 16, border: "none", cursor: "pointer", transition: "transform 0.15s ease, box-shadow 0.15s ease",
    WebkitTapHighlightColor: "transparent", gap: 6, minWidth: 0, boxSizing: "border-box",
  };

  const modalCloseBtnStyle: React.CSSProperties = {
    background: C.bg, border: "none", width: 32, height: 32, borderRadius: "50%", cursor: "pointer",
    display: "flex", alignItems: "center", justifyContent: "center", color: C.textTertiary, fontSize: 16,
  };
  ...
  return (
    <div style={{ minHeight: "100vh", width: "100%", overflowX: "hidden", background: C.bg, fontFamily: "-apple-system, BlinkMacSystemFont, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 24px", background: "rgba(255,255,255,0.8)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: `1px solid ${C.separator}`, position: "sticky", top: 0, zIndex: 10, flexShrink: 0, flexWrap: "wrap", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 200 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", minWidth: 0 }}>
            ...
            {phone && <a href={`tel:${phone}`} style={{ fontSize: 13, color: C.textTertiary, textDecoration: "none", display: "flex", alignItems: "center", gap: 4, whiteSpace: "nowrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={() => navigate("/trainer/settings")} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: 9, background: "rgba(118, 118, 128, 0.12)", border: "none", cursor: "pointer", color: C.textPrimary, transition: "background 0.2s" }} title="Settings">
          ...
          <button onClick={handleLogout} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: 9, background: C.redBg, border: "none", cursor: "pointer", color: C.red, transition: "background 0.2s" }} title="Logout">
      ...
      <div style={{ flex: 1, padding: "32px 24px", width: "100%", maxWidth: "1200px", margin: "0 auto", boxSizing: "border-box" }}>
        ...
        {loading && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 24 }}><SkeletonCourseCard /><SkeletonCourseCard /><SkeletonCourseCard /></div>}

        {!loading && courses.length === 0 && (
          <div style={{ background: C.card, borderRadius: 24, padding: 48, textAlign: "center", color: C.textTertiary, boxShadow: C.shadow, border: `1px solid ${C.separatorLight}` }}>
            <h3 style={{ margin: "0 0 8px 0", color: C.textPrimary, fontSize: 20, fontWeight: 700 }}>No Courses Assigned</h3>
            <p style={{ margin: 0, fontSize: 15 }}>Your administrator has not assigned any courses to you yet.</p>
          </div>
        )}
        ...
        {!loading && courses.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(340px, 1fr))", gap: 24 }}>
            {courses.map((course) => {
              ...
              return (
                <div key={course.id} style={{ background: C.card, borderRadius: 24, boxShadow: C.shadow, overflow: "hidden", display: "flex", flexDirection: "column", border: `1px solid ${C.separatorLight}`, transition: "box-shadow 0.3s ease, transform 0.3s ease" }}
                  onMouseEnter={(e) => { e.currentTarget.style.boxShadow = C.shadowHover; e.currentTarget.style.transform = "translateY(-4px)"; }}
                ...
                >
                  {course.logo && (
                    <div style={{ position: "relative", width: "100%", height: 200 }}>
                      ...
                      <div style={{ position: "absolute", bottom: 0, left: 0, width: "100%", height: "60%", background: "linear-gradient(to top, rgba(0,0,0,0.6), transparent)" }} />
                      <div style={{ position: "absolute", top: 16, right: 16, background: "rgba(0,0,0,0.6)", color: "#fff", padding: "6px 14px", borderRadius: 20, fontSize: 13, fontWeight: 600, backdropFilter: "blur(8px)" }}>
                        👥 {learnerCount}
                      </div>
                    </div>
                  )}
                  <div style={{ padding: 24, borderBottom: `1px solid ${C.separatorLight}`, flex: 1, display: "flex", flexDirection: "column" }}>
                    ...
                    <button onClick={() => setScheduleCourse(course)} style={{ alignSelf: "flex-start", marginTop: 16, background: "none", border: "none", color: C.orange, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", gap: 6, fontSize: 14, fontWeight: 600 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                      Schedule Session
                    </button>
                  </div>
                  <div style={{ padding: 16, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                    <button onClick={() => navigate(`/trainer/live/${course.id}`)} style={{ ...iconBtnStyle, background: C.medBlueBg, color: C.medBlue }} onMouseEnter={(e) => e.currentTarget.style.transform = "scale(1.05)"} onMouseLeave={(e) => e.currentTarget.style.transform = "none"}>
                    ...
                    <button onClick={() => navigate(`/trainer/upload/${course.id}`)} style={{ ...iconBtnStyle, background: C.greenBg, color: C.green }} onMouseEnter={(e) => e.currentTarget.style.transform = "scale(1.05)"} onMouseLeave={(e) => e.currentTarget.style.transform = "none"}>
                    ...
                    <button onClick={() => navigate(`/trainer/assignments/${course.id}`)} style={{ ...iconBtnStyle, background: C.purpleBg, color: C.purple }} onMouseEnter={(e) => e.currentTarget.style.transform = "scale(1.05)"} onMouseLeave={(e) => e.currentTarget.style.transform = "none"}>
                      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg>
                      <span style={{ fontSize: 11, fontWeight: 600 }}>Assignments</span>
                    </button>
                    <button onClick={() => openAttendance(course)} style={{ ...iconBtnStyle, background: C.orangeBg, color: C.orange }} onMouseEnter={(e) => e.currentTarget.style.transform = "scale(1.05)"} onMouseLeave={(e) => e.currentTarget.style.transform = "none"}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 600 }}>Attendance</span>
                    </button>
                  </div>
                </div>
              );
        ...
        <div onClick={() => setAttendanceCourse(null)} style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16, boxSizing: "border-box" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, borderRadius: 24, width: "100%", maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 50px rgba(0,0,0,0.2)" }}>
            <div style={{ padding: 24, borderBottom: `1px solid ${C.separatorLight}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <h2 style={{ margin: "0 0 4px 0", fontSize: 20, fontWeight: 700 }}>{attendanceCourse.name}</h2>
                <p style={{ margin: 0, fontSize: 14, color: C.textTertiary }}>Learners ({assignedLearners.length})</p>
              </div>
              <button onClick={() => setAttendanceCourse(null)} style={modalCloseBtnStyle}><CloseIcon /></button>
            </div>
            <div style={{ padding: 24, overflowY: "auto" }}>
              {loadingLearners ? <p style={{ textAlign: "center", color: C.textTertiary }}>Loading...</p> : 
               assignedLearners.length === 0 ? <p style={{ textAlign: "center", color: C.textTertiary }}>No learners enrolled yet.</p> : 
               <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {assignedLearners.map((learner) => (
                  <div key={learner.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: C.bg, borderRadius: 12 }}>
                    {learner.profilePic ? <img src={learner.profilePic} alt="" style={{ width: 36, height: 36, borderRadius: "50%", objectFit: "cover" }} /> : <div style={{ width: 36, height: 36, borderRadius: "50%", background: C.medBlueBg, color: C.medBlue, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 600 }}>{learner.username?.charAt(0).toUpperCase()}</div>}
                    <div style={{ flex: 1 }}>
                      <span style={{ fontWeight: 600, color: C.textPrimary, fontSize: 15, display: "block" }}>{learner.username}</span>
                      <span style={{ fontSize: 13, color: C.textTertiary }}>{learner.phone || "No phone"}</span>
                    </div>
                  </div>
                ))}
               </div>}
      ...
      {scheduleCourse && (
        <div onClick={() => setScheduleCourse(null)} style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16, boxSizing: "border-box" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, borderRadius: 24, width: "100%", maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 50px rgba(0,0,0,0.2)" }}>
            <div style={{ padding: 24, borderBottom: `1px solid ${C.separatorLight}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <h2 style={{ margin: "0 0 4px 0", fontSize: 20, fontWeight: 700 }}>📅 {scheduleCourse.name}</h2>
                <p style={{ margin: 0, fontSize: 14, color: C.textTertiary }}>Create sessions</p>
              </div>
              <button onClick={() => setScheduleCourse(null)} style={modalCloseBtnStyle}><CloseIcon /></button>
            </div>
            <div style={{ padding: 24, overflowY: "auto" }}>
              ...
              {schedules.length === 0 ? <p style={{ textAlign: "center", color: C.textTertiary, padding: 20 }}>No sessions scheduled yet.</p> : 
               <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {schedules.map((sch) => (
                  <div key={sch.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: C.bg, borderRadius: 12 }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, color: C.textPrimary, fontSize: 15, marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{sch.title}</div>
                      {sch.description && <div style={{ fontSize: 13, color: C.textTertiary, marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{sch.description}</div>}
                      <div style={{ fontSize: 13, color: C.textTertiary }}>{new Date(sch.scheduled_at).toLocaleString()}</div>
                    </div>
                    <button onClick={() => handleDeleteSchedule(sch.id)} style={{ background: C.redBg, border: "none", color: C.red, cursor: "pointer", fontSize: 13, fontWeight: 600, padding: "6px 12px", borderRadius: 8, marginLeft: 12, flexShrink: 0 }}>Delete</button>
                  </div>
                ))}
               </div>}
            </div>
